---
title: "Gene Functional Enrichment Analysis — *E. coli* MG1655"
author: ""
date: "`r Sys.Date()`"
output:
  html_document:
...
- **Query set** — significant DE genes (padj < 0.05, |LFC| >= 1), converted to gene symbols,
  split by direction (up- and down-regulated)
...
direction of fold change. Running ORA separately on up- and down-regulated genes partially
addresses this, and is why we split the query set below. For a fully ranked approach,
see Part 2 (GSEA).
:::

### Prepare Input

We split significant genes by direction before running ORA. The background remains all
genes tested by DESeq2, regardless of direction.
...
cat("Background (all tested) :", length(all_genes), "\n")
```
...
ORA is run twice — once for up-regulated genes and once for down-regulated genes — using
...
cat("Down — pathways tested:", nrow(ora_down),
...
cat("Significant down pathways:", nrow(ora_down_sig), "\n")
```

**Up-regulated genes — enriched pathways**

```{r ora-table-up, echo=TRUE, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE}
DT::datatable(
  ora_up_sig %>%
...
```{r ora-table-down, echo=TRUE, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE}
DT::datatable(
  ora_down_sig %>%
    select(ontology_id, ontology_name, nr_common_with_tested_elements,
           nr_common_with_background_elements, p_value, eFDR) %>%
...
```{r ora-barplot-all, echo=TRUE, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE}
...
ora_barplot_all
suppressMessages(ggplotly(ora_barplot_all))
```
...
cat("Down pathways after cleaning:", nrow(ora_down_clean), "\n")
```

**Visualise ORA Results**

### Faceted Bar Plot — Enriched Pathways by Direction

Up- and down-regulated results are shown in separate facets, each ordered independently
by significance. The gene count (number of DE genes hitting each pathway) is printed at
the end of each bar.
...
Point colour encodes eFDR; position on x encodes how many DE genes from the query set
hit each pathway. Facets separate the two directions.
